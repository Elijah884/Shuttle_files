...
class Controller:
	...
	def modbus_fail_read_handler(self, ADDR, WORD):
		"""
		Parameters:
		ADDR - the address of the register being read from
		WORD - the number of registers being read

		This function ensures that the ZLAC8015D properly returns any exceptions when we 
		...
		while not read_success:
			...
			try:
				if isinstance(result, ModbusException):
					# Because of the way read_holding_registers handles exceptions, we must
					# flag and raise them on our own
					print(result)
					raise Exception
				for i in range(WORD):
	...
	def get_fault_code(self):
		...
		# Convert the flags into the codes described in the fault table
	...
	def deg_to_32bitArray(self, deg):
		"""
		Helper function for set_relative_angle. Generates a 32 bit array from an angle
		"""
	...
	def set_relative_angle(self, ang_L, ang_R):
		"""
		Sets the relative angle for both wheels to travel to a certain point

		Parameters:
		ang_L - the angle from the left wheel to the desired point in degrees
		ang_R - the angle from the right wheel to the desired point in degrees
		"""
	...
	def get_wheels_travelled(self):
		"""
		Returns the total number of times the wheel has rotated
		"""
	...
	def get_wheels_tick(self):
		"""
		Returns the current encoder tick from the motors
		"""
	...
	def get_currents(self):
		...
		registers = self.modbus_fail_read_handler(self.L_CUR, 2)
		left_current = registers[0]
		right_current = registers[1]
		if left_current >=0x8000:
			left_current -= 0x10000
		if right_current >=0x8000:
			right_current -= 0x10000
